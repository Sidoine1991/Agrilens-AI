...
def analyze_image_final(image, prompt=""):
    """Analyse une image avec le modèle Gemma 2B (version finale)"""
    ...
    try:
        ...
        # Version finale : analyser seulement le texte avec description d'image
        if prompt:
...
with st.sidebar:
    ...
    if st.button("Charger le modèle Gemma 2B (version finale)", type="primary"):
...
with tab1:
    ...
    try:
        ...
        if uploaded_file is not None:
            try:
                # Ouvrir l'image avec gestion d'erreur
                ...
                if st.button("🔬 Analyser avec l'IA", disabled=not st.session_state.model_loaded, type="primary"):
                    if not st.session_state.model_loaded:
                        ...
                        with st.spinner("🔍 Analyse en cours..."):
                            result = analyze_image_final(image, question)
...
with tab2:
    ...
    if st.button("🧠 Analyser avec l'IA", disabled=not st.session_state.model_loaded, type="primary"):
        if not st.session_state.model_loaded:
            ...
            with st.spinner("🔍 Analyse en cours..."):
                result = analyze_text_final(text_input)
