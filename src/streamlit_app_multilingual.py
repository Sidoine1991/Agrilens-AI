...
def analyze_image_multilingual(image, prompt=""):
    ...
    Inclut des assertions et des prints pour le débogage.
    """
    ...
    try:
        ...
        # --- DEBUG : Préparation du prompt final et vérifications ---
        # Le token <image> est supposé être inséré par apply_chat_template lors du traitement de {"type": "image", "image": image}
        # dans les messages. Donc le user_instruction ne devrait PAS contenir <image> directement.
        # Si on l'ajoute dans user_instruction, il y aurait un double comptage du token image.
        final_user_prompt = user_instruction 

        # Vérifications explicites avant de passer au processeur
        print("\n--- Début du Debugging Multimodal ---")
        print("🧪 Prompt texte utilisateur (sans <image> explicite ici) :", final_user_prompt)
        assert image is not None, "Assertion Failed: Aucune image fournie."
        assert hasattr(image, "size"), f"Assertion Failed: Image invalide (doit être une instance PIL.Image ou équivalent). Type reçu: {type(image)}"
        print("🖼️ Image fournie (dimensions) :", image.size)
        print("Modèle device:", model.device)
        
        messages = [
            {"role": "system", "content": [{"type": "text", "text": system_message}]},
            {"role": "user", "content": [
                {"type": "image", "image": image},
                {"type": "text", "text": final_user_prompt} # Utiliser le prompt texte utilisateur
            ]}
        ]
        
        # --- DEBUG : Avant processor.apply_chat_template ---
        print("\n--- Avant processor.apply_chat_template ---")
        print("Messages passés au processeur :", messages)
        ...
        # --- DEBUG : Après processor.apply_chat_template ---
        print("\n--- Après processor.apply_chat_template ---")
        print("Keys dans 'inputs' après apply_chat_template:", inputs.keys())
        print("Shape de input_ids:", inputs["input_ids"].shape)
        if "pixel_values" in inputs:
            print("Shape de pixel_values:", inputs["pixel_values"].shape)
        else:
            print("pixel_values non trouvés dans inputs.")
        print("input_ids (premiers 10) :", inputs["input_ids"][0, :10])
        ...
        # --- DEBUG : Avant model.generate ---
        print("\n--- Avant model.generate ---")
        print("input_ids sur device:", inputs["input_ids"].device)
        print("attention_mask sur device:", inputs["attention_mask"].device)
        if "pixel_values" in inputs:
            print("pixel_values sur device:", inputs["pixel_values"].device)
        
        with torch.inference_mode():
            # LA MODIFICATION CRUCIALE POUR LE BUG #2751
            # Passer explicitement 'input_ids' et 'attention_mask' et 'pixel_values'
            # pour s'assurer que la comparaison interne du modèle est faite sur les bons tenseurs.
            generate_kwargs = {
            ...
            if "pixel_values" in inputs: # Assurez-vous de passer pixel_values si le processeur les a générés
                generate_kwargs["pixel_values"] = inputs["pixel_values"]
            
            generation = model.generate(**generate_kwargs)

            # --- DEBUG : Après model.generate ---
            print("\n--- Après model.generate ---")
            print("Shape de la génération brute:", generation.shape)
        ...
        # --- DEBUG : Fin du Debugging Multimodal ---
        print("\n--- Fin du Debugging Multimodal ---")
...
"""
        ...
        error_msg = str(e)
        print(f"--- DEBUG: Exception capturée dans analyze_image_multilingual: {error_msg} ---")
...
def analyze_text_multilingual(text):
    ...
    try:
        ...
        device = getattr(model, 'device', 'cpu')
        if hasattr(inputs, 'to'):
            inputs = {k: v.to(device) for k, v in inputs.items()} # Déplacer toutes les clés du dictionnaire

        input_len = inputs["input_ids"].shape[-1]
        
        with torch.inference_mode():
            # Pour l'analyse de texte, on peut déballer ou passer explicitement
            # Ici, nous passons explicitement pour la cohérence
...
with tab3:
    ...
    manual_content = {
        ...
        *   **Redimensionnement** : Oversized images are automatically resized for processing optimization.
