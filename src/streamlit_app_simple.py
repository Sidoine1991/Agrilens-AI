...
def analyze_image_simple(image, prompt=""):
    """Analyse une image avec le modèle Gemma 3n"""
    if not st.session_state.model_loaded:
        return "❌ Modèle non chargé. Veuillez le charger dans les réglages."
    
    try:
        model, processor = st.session_state.model
        ...
        if prompt:
            ...
            user_content = [
                {"type": "image", "image": image},
                {"type": "text", "text": "Décris la maladie présente sur cette plante et donne des recommandations pratiques."}
            ]
    ...
    if st.button("Charger le modèle Gemma 3n multimodal", type="primary"):
        with st.spinner("Chargement du modèle..."):
            ...
            if model and processor:
                st.session_state.model = (model, processor)
        ...
        if st.button("🔬 Analyser avec l'IA", disabled=not st.session_state.model_loaded, type="primary"):
            if not st.session_state.model_loaded:
                ...
                with st.spinner("🔍 Analyse en cours..."):
                    result = analyze_image_simple(image, question)
